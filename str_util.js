 ...
 */
str_util.fill = function (width, fill_char) {
    if (!width)
        throw new Error("Invalid call to 'str_util.fill', the 'width' parameter is " +
            "not optional.");
...
/**
 * Capitalizes the first letter of every word in the given 'str'.
 * @param {String} str  String to be capitalized.
 * @returns {String} Returns a string with the first letter of every word capitalized.
 */
str_util.capitalize = function (str) {
    if (!str || !str.toString)
        throw new Error("Invalid call to 'str_util.capitalize', an invalid parameter " +
            "was passed.");
...
/**
 * Replaces tokens of the form {N} where N indicates the index of the value to use for
 * replacement. Replacement values are passed as parameters to the function.
 * @param {String} str
 * String to be formatted. All the known tokens present in 'str' are replaced with the
 * values given by the rest of the function arguments.
 * @param {*} [subs...]
...
*/
str_util.format = function(str, subs /*, [subs2, ...]*/) {
    if (0 === arguments.length)
        throw new Error("Invalid call to 'str_util.format', no string was supplied");

    if (!str.toString)
        throw new Error("Invalid call to 'str_util.format', the 'str' parameter must " +
            "be a 'String' or support the 'toString' method.");
...
/**
 * Returns an <b>Error</b> object with the text <b>str</b> expanded via the <b>format</b>
 * function.
 * @param {String} str
 * String to be formatted. All the known tokens present in 'str' are replaced with the
 * values given by the rest of the function arguments.
 ...
 * @return {Error}
 */
str_util.formatError = function (str, subs /*, [subs2, ...]*/) {
   return new Error(str_util.format.call(this, arguments));
};


/**
 * Expands substitutable tokens (text surrounded by handlebars) by invoking the callback
 * function. Parameters can be given along within the substitution token to customize the
 * behaviour of the substitution function, these parameters are separated by a comma and
 * can be enclosed within double quotes, for instance the token {{ date, "yyyy/MM/dd" }}
 * could be used to tell the substitution function to format that date using the given
 * date formatting string. If the callback function returns 'null', the token is left
 * alone.<br><br>
 *
 ...
 *
 * @param {String} str
 * String to be expanded. Tokens found in this string are given to the callback function
 * for substitution.
 ...
 *  }, new Date());
 */
str_util.expand = function (str, subst_fn, arg /*[arg2, ...]*/) {
 ...
 * 'alignC'.<br> Pads the given string (str) to fit in the given 'width'. Padding is added
 * to the beginning, the end or both depending on the value of the 'alignment' attribute,
 * possible values are: <b>"left"</b>, <b>"right"</b> and <b>"center"</b>. The padding
 * character can be defined through the 'fill_char' argument, if the argument is omitted,
 * an space character (" ") is used. If the given string is longer than the given width,
 * no padding is added and the original string is returned back to the caller.
 * @param {String} str
 ...
 */
str_util.align = function (str, width, alignment, fill_char) {
    ...
    switch(alignment) {
        ...
        default:
            throw new Error("Invalid call to 'str_util.align', the aignment method is " +
                "not supported.");
    }
};
 ...
 * beginning and end of the string such that 'str' is centered within the given number of
 * characters. The padding character can be defined through the <b>fill_char</b> argument,
 * if the argument is omitted, the space character (" ") is used. If the given string is
 * longer than the given width, no padding is added and the original * string is returned
 * back to the caller.
 * @param {String} str
 * The string (or string serializable object) to be aligned.
 * @param {Number} width
 ...
 * Returns the padded string.
 */
str_util.alignC = function (str, width, fill_char) {
    return str_util.align(str, width, "center", fill_char);
};


/**
 * Pads the given string (str) to fit in the given 'width'. Padding is added to the end of
 * the string. The padding character can be defined through the <b>fill_char</b> argument,
 * if the argument is omitted, the space character (" ") is used. If the given string is
 * longer than the given width, no padding is added and the original string is returned
 * back to the caller.
 * @param {String} str
 ...
 * Returns the padded string.
 */
str_util.alignL = function (str, width, fill_char) {
    return str_util.align(str, width, "left", fill_char);
};
 ...
 * <b>fill_char</b> argument, if the argument is omitted, the space character (" ") is
 * used. If the given string is longer than the given width, no padding is added and the
 * original string is returned back to the caller.
 * @param {String} str
 * The string (or string serializable object) to be aligned.
 * @param {Number} width
 * The number of characters the resulting string should have.
 ...
 * Returns the padded string.
 */
str_util.alignR = function(str, width, fill_char) {
    return str_util.align(str, width, "right", fill_char);
};


/**
 * Creates and object by de-serializing the string. The string must contain a valid JSON
 * document or an error will be thrown by the "eval" function. To use the JSON safe
 * parser, pass a parameter of "null" in "reviver", or  valid reviver function. See
 * "json.js" for more examples and usage information.
 * @param {String} doc
 ...
 */
str_util.toObject = function (doc, reviver)
{
 ...
 */
str_util.crc32 = function (str) {
    if (!str)
        throw new Error("Invalid call to 'str_util.crc32', the str parameter is not " +
            "optional.");

    if (!str.toString)
        throw new Error("Invalid call to 'str_util.align', the 'str' parameter must " +
            "be a 'String' or support the 'toString' method.");
    str = str.toString();       // Make sure we have a string to work with
 ...
 * @param {Date} [date]
 * Optional date to be used for substitution, if not provided, the current date/time
 * is used.
 * @param {Number} [width]
 * Width used for alignment
 * @returns {String}
 */
str_util.subst.date = function (str, date, width) {
    if (undefined === str)
        throw new Error("Invalid call to 'str_util.subst.date', the 'str' argument " +
            "is not optional.");

    return str_util.expand(str, function (token, dt) {
        if ("date" === token.name) {
            if (Object.isType(token.args[0], String))
                return str_util.align(dt.format(token.args[0]), width, token.align);

            return str_util.align(dt, width, token.align);
        }
    }, date || new Date());
};

/**
 * Substitutes the a token with an object property. Properties of child objects can be
 * specified as tokens using dot notation, e.g. "{{process.path}}".
 * @param {string} str
 * String where token substitution is to be applied.
 * @param {Object} obj
 ...
 * @return {String}
 */
str_util.subst.prop = function (str, obj, width, cb) {
    if (!obj || undefined === obj)
        throw new Error("Invalid call to 'str_util.subst.prop', the 'obj' and 'obj' " +
    ...
    return str_util.expand(str, function (token, obj) {
        let val = getPropValue(obj, token.name.split("."), token.args, cb);
        if (undefined !== val && null !== val)
            return str_util.align(val, width, token.align);
    }, obj);
 ...
 * Expanded string.
 */
str_util.subst.env = function(str) {
 ...
 * @returns {string}
 */
str_util.buildTable = function (table_data, width) {
    ...
    if (Object.isType(table_data, Array)) {
        ...
        table_data.forEach(function (val) {
            res += str_util.buildTable(val, width) + os.EOL;
        });
    ...
    tables.forEach(function (t) {
        if (Object.isType(t, String)) {
            if ("=" === t)
                res += str_util.fill(unadj_width, "=") + os.EOL;
            else {
                let align = "left";
                if ("-" === t.charAt(0)) {
                    t = t.substr(1);
